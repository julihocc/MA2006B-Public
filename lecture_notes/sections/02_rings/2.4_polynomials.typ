...
#solution[
  ...
  Consider the ideal in $D[x]$ generated by $x$ and $a$, denoted by $I = chevron.l x, a chevron.r$.
  
  Since $D[x]$ is a PID, this ideal must be generated by a single polynomial, say $d(x) in D[x]$. Thus, $I = chevron.l d(x) chevron.r$.
  
  Since $a in I$, there exists some $f(x) in D[x]$ such that $a = f(x)d(x)$. Because $a$ is a non-zero constant (degree 0) and $D$ is an integral domain, the degrees of $f(x)$ and $d(x)$ must both be 0. Thus, $d(x) = d in D$ is a constant.
  
  Since $x in I$, there exists some $g(x) in D[x]$ such that $x = g(x)d$.
  
  The leading coefficient of $g(x)d$ is the leading coefficient of $g(x)$ multiplied by $d$. For the product to be exactly $x$, $g(x)$ must have degree 1, say $g(x) = c x + b$. Then $x = (c x + b)d = (c d)x + b d$.
  
  Equating coefficients, this implies $c d = 1$ and $b d = 0$. Since $c d = 1$, $d$ must be a unit in $D$.
  
  Because $d$ is a unit, the ideal it generates is the entire ring: $chevron.l d chevron.r = chevron.l x, a chevron.r = D[x]$. Therefore, the constant polynomial $1$ is in $chevron.l x, a chevron.r$.
  
  By the definition of the ideal $chevron.l x, a chevron.r$, there exist polynomials $p(x), q(x) in D[x]$ such that:
  $ 1 = p(x)x + q(x)a $
  
  Evaluating this polynomial equation at $x = 0$, we get:
  $ 1 = p(0)(0) + q(0)a $
  $ 1 = q(0)a $
  
  Since $q(0) in D$, we have found an element $q(0)$ that is the multiplicative inverse of $a$.
  
  Since $a$ was an arbitrary non-zero element of $D$, every non-zero element in $D$ is invertible, which proves that $D$ is a field.
]
