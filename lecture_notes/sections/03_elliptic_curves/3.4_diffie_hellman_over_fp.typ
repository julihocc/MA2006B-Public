...
#algorithm("Elliptic-Curve Diffie-Hellman")[
  *Goal.* Alice and Bob want to agree on the same point $S$ in the subgroup generated by $G$, without sending their private scalars to each other. This shared point can later be used to derive a symmetric key.

  *Public input.* An elliptic curve $E(FF_p)$ and a base point $G in E(FF_p)$ of order $n$.

  *Private input.* Alice has a secret scalar $a$ and Bob has a secret scalar $b$, with
  $ 1 <= a,b <= n-1. $

  *Output.* Alice and Bob should both obtain the same shared point
  $ S in chevron.l G chevron.r. $

  *Key generation.* Alice computes her public point
  $ A = a G. $
  Bob computes his public point
  $ B = b G. $

  *Public exchange.* Alice sends Bob the public point
  $ A. $
  Bob sends Alice the public point
  $ B. $

  *Shared-point computation.* Alice uses Bob's public point and computes
  $ S_A = a B. $
  Bob uses Alice's public point and computes
  $ S_B = b A. $

  Since $B=b G$ and $A=a G$,
  $ S_A = a B = a(b G) = (a b)G, $
  while
  $ S_B = b A = b(a G) = (b a)G. $
  Because integer multiplication commutes, $(a b)G=(b a)G$. Thus both parties obtain the same shared point
  $ S = (a b)G. $
]
